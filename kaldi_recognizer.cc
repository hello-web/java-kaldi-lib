...
KaldiRecognizer::KaldiRecognizer(KaldiModel &model) : model_(model) {
...
//{
    ...
    const float kFramesPerSecond = 0.03f;
...
//    ss << "{\"result\" : [ ";
    ...
    for (int i = 0; i < size; i++) {
        ss << "{\"data\":{\"note\": \"" << model_.word_syms_->Find(words[i]) << "\"}, \"start\" : " << (frame_offset + times[i].first) * kFramesPerSecond << "," <<
                " \"end\" : " << (frame_offset + times[i].second) * kFramesPerSecond << ", \"conf\" : " << conf[i] << " , \"drag\": false, \"resize\": false}";
        if (i != size - 1)
            ss << ",\n";
        else
            ss << "\n";
